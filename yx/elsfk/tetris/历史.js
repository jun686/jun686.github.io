...
// 作者并非从事计算机行业，代码风格比较随意。

//右上角的微微闪烁的感叹号，显示游戏主循环的工作状态，原本这个东西是用来找bug的，现在保留下来。
const stepCounter = {
  msg: document.querySelector('#step-counter'),
  step: 0,
  running: function () {
    this.msg.style.color = (this.step = this.step ? 0 : 1) ? '#ddd' : '#bbb';
  }
}
...
function setMainCanvas() {
  ...
  scanvas.width = scanvasWidth = cs * 4;
...
let gameLevel = 1;
// 1 ~ 25
let timeList = [1000,850,722,613,521,442,375,318,270,229,194,164,139,118,100,85,72,61,51,43,36,30,25,21,17];

function changeLevalAndDisplay(line) {

  gameLevel = line < 10 ? 1 : Math.ceil(line / 10);

  let speed = timeList[gameLevel - 1];
  //最后一关，后面没了！
  if (!speed) {
    gameLevel = 26;
    speed = 15
...
QS('#rcmd-close').addEventListener('click', function () {
  inTop10Check();
})
...
['top10', 'gameover', 'inputTop10', 'pausebox', 'setting', 'recommend', 'rstgaming'].forEach(key => {
...
function displayOptKey() {
  ...
  if (localData.offsound) {
    offsound.checked = true;
  } else {
    offsound.checked = false;
  }
}
...
QS("#u-enterNameBT").addEventListener("click", function () {
  ...
  checkDataAndSave([name || "刺客!", gameScore, gameLevel, finishLine]);
  popupWin(false)
});
...
QS("#opt-bt-yes").addEventListener("click", function () {
  ...
  if (offsound.checked) {
    localData.offsound = true;
  } else {
    localData.offsound = false;
  }
